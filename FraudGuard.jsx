    ...
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", maxWidth: 680, margin: "0 auto", padding: "0 0 2rem" }}>
      <div style={{ padding: "1.5rem 0 1.25rem", borderBottom: `2px solid ${PINK[400]}`, marginBottom: "1.25rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
          <div style={{ width: 34, height: 34, borderRadius: "50%", background: PINK[400], display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <h1 style={{ margin: 0, fontSize: 20, fontWeight: 500, color: "#1A1A1A" }}>
                Fraud<span style={{ color: PINK[400] }}>Guard</span>
              </h1>
              <span style={{ fontSize: 10, background: PINK[50], color: PINK[600], padding: "2px 7px", borderRadius: 20 }}>DEMO</span>
            </div>
            <p style={{ margin: 0, fontSize: 12, color: "#666666" }}>Detect AI content · fake products · fraudulent sellers</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#FFFFFF", border: "0.5px solid #E5E5E5", borderRadius: "12px", overflow: "hidden", marginBottom: "1rem" }}>
        <div style={{ padding: "10px 14px", borderBottom: "0.5px solid #E5E5E5", background: "#F9F9F9", display: "flex", justifyContent: "space-between" }}>
          <span style={{ fontSize: 12, color: "#666666", fontWeight: 500 }}>{tab.icon} {tab.desc}</span>
          {activeTab === "image" && (
            <button onClick={() => fileRef.current?.click()} style={{ padding: "4px 10px", fontSize: 11, border: `0.5px solid ${PINK[300]}`, borderRadius: "6px", background: PINK[50], color: PINK[600], cursor: "pointer" }}>
              Upload file
            </button>
          )}
        ...
        {activeTab === "image" && imagePreview && (
          <div style={{ padding: "10px 14px", borderBottom: "0.5px solid #E5E5E5", display: "flex", alignItems: "center", gap: 10 }}>
            <img src={imagePreview} alt="Preview" style={{ height: 60, width: 60, objectFit: "cover", borderRadius: "6px" }} />
            <div>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 500 }}>{imageFile?.name}</p>
              <p style={{ margin: 0, fontSize: 11, color: "#999999" }}>{(imageFile?.size / 1024).toFixed(0)} KB</p>
            </div>
            <button onClick={clearImage} style={{ marginLeft: "auto", fontSize: 11, color: "#999999", border: "none", background: "none", cursor: "pointer" }}>Remove</button>
          </div>
        )}

        <textarea value={inputs[activeTab]} onChange={(e) => { setInputs((i) => ({ ...i, [activeTab]: e.target.value })); if (activeTab === "image" && e.target.value) clearImage(); }} placeholder={imageFile ? "Image ready — click Analyze" : placeholders[activeTab]} disabled={!!(activeTab === "image" && imageFile)} style={{ width: "100%", minHeight: activeTab === "text" ? 140 : 100, padding: "12px 14px", border: "none", outline: "none", resize: "vertical", fontSize: 13, background: "transparent", color: "#1A1A1A" }} />

        <div style={{ padding: "10px 14px", borderTop: "0.5px solid #E5E5E5", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 11, color: "#999999" }}>{tab.hint}</span>
          <button onClick={() => analyze(activeTab)} disabled={isLoading || !canAnalyze} style={{ padding: "8px 18px", background: isLoading || !canAnalyze ? "#F0F0F0" : PINK[400], color: isLoading || !canAnalyze ? "#999999" : "white", border: "none", borderRadius: "6px", fontSize: 13, fontWeight: 500, cursor: isLoading || !canAnalyze ? "default" : "pointer", minWidth: 110 }}>
      ...
      {isLoading && (
        <div style={{ padding: "1.5rem", background: PINK[50], border: `0.5px solid ${PINK[200]}`, borderRadius: "12px", textAlign: "center" }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: PINK[600] }}>ANALYZING</div>
          <div style={{ fontSize: 13, color: PINK[800] }}>Scanning for fraud signals...</div>
        </div>
      )}

      {result && !isLoading && (
        <div style={{ background: "#FFFFFF", border: "0.5px solid #E5E5E5", borderRadius: "12px", overflow: "hidden" }}>
          {result.error ? (
            <div style={{ padding: "1.25rem", color: "#A32D2D" }}>{result.summary}</div>
          ) : (
            <>
              <div style={{ padding: "1rem 1.25rem", borderBottom: "0.5px solid #E5E5E5", background: "#F9F9F9", display: "flex", justifyContent: "space-between", flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 500, background: verdictStyle(result.verdict).bg, color: verdictStyle(result.verdict).color }}>{result.verdict}</span>
                  <span style={{ fontSize: 12 }}>{result.confidence}% confidence</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 10, color: "#999999" }}>RISK SCORE</span>
                  <div style={{ width: 80, height: 5, background: "#E5E5E5", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${result.riskScore}%`, background: riskBarColor(result.riskScore) }} />
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 500, color: riskBarColor(result.riskScore) }}>{result.riskScore}%</span>
                </div>
              </div>
              <div style={{ padding: "1rem 1.25rem", borderBottom: "0.5px solid #E5E5E5" }}>
                <p style={{ margin: 0, fontSize: 13, lineHeight: 1.7 }}>{result.summary}</p>
              </div>
              {result.signals && result.signals.length > 0 && (
                <div style={{ padding: "1rem 1.25rem" }}>
                  <p style={{ margin: "0 0 10px", fontSize: 10, fontWeight: 500, color: "#999999" }}>DETECTED SIGNALS</p>
                  {result.signals.map((signal, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 7 }}>
                      <span style={{ color: PINK[400], fontSize: 8, marginTop: 5 }}>◆</span>
                      <span style={{ fontSize: 13, color: "#666666" }}>{signal}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      {!result && !isLoading && (
        <div style={{ textAlign: "center", padding: "2rem 1rem" }}>
          <div style={{ width: 52, height: 52, borderRadius: "50%", background: PINK[50], border: `0.5px solid ${PINK[200]}`, margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, color: PINK[300] }}>{tab.icon}</div>
